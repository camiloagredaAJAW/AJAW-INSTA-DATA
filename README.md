...
## Chatwoot API Channel outbound replies

`POST /integrations/chatwoot/webhook` receives Chatwoot API Channel message webhooks and sends valid agent replies back to Instagram.

Configure each Chatwoot API Channel inbox Webhook URL as:

```txt
https://<public-host>/integrations/chatwoot/webhook
```

When this service creates a Chatwoot API Channel inbox during activation, it sets that webhook URL automatically from `APP_BASE_URL`. Existing inboxes should be checked manually in Chatwoot if they were created before v1.1.0.

Outbound routing processes only Chatwoot `message_created` webhooks that are public outgoing Instagram replies. Incoming messages, private notes, and non-Instagram contacts return `200 ignored` so Chatwoot does not mark inbound messages as failed. Some Chatwoot deployments do not sign API Channel webhooks; unsigned outgoing webhooks are accepted for compatibility, while present-but-invalid signature headers are rejected.

To prevent loops, Instagram echo webhooks generated by messages sent from Chatwoot are ignored using recent outbound fingerprints (`recipientId + text`) and, when available, Meta message ids.
...
## v1.1.0 verified outbound replies

The v1.1.0 release adds verified Chatwoot → Instagram replies:

- Chatwoot API Channel webhook URL: `/integrations/chatwoot/webhook`.
- Agent replies from Chatwoot are sent to Instagram through Graph API `/messages`.
- Inbound Instagram DMs no longer show Chatwoot `Failed to send` caused by webhook acknowledgement failures.
- Instagram echo webhooks from Chatwoot-originated replies are suppressed to avoid duplicate sends.
